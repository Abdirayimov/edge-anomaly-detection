...
TrtEngine::TrtEngine(const std::string& engine_path) : impl_(std::make_unique<Impl>()) {
    ...
    for (std::int32_t i = 0; i < n; ++i) {
        ...
        BindingInfo info;
        info.name = name;
        for (std::int32_t k = 0; k < dims.nbDims; ++k) info.shape.push_back(dims.d[k]);
        ...
        if (info.volume > 0 && info.element_size > 0) {
            void* ptr = nullptr;
            EANOM_CUDA_CHECK(cudaMalloc(&ptr, info.volume * info.element_size));
...
void TrtEngine::copy_input(const std::string& name, const void* host_src, std::size_t bytes,
    ...
    EANOM_CUDA_CHECK(cudaMemcpyAsync(dst, host_src, bytes, cudaMemcpyHostToDevice, stream));
}
...
void TrtEngine::copy_output(const std::string& name, void* host_dst, std::size_t bytes,
    ...
    EANOM_CUDA_CHECK(cudaMemcpyAsync(host_dst, src, bytes, cudaMemcpyDeviceToHost, stream));
}
